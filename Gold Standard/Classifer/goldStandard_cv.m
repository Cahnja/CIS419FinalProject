...
% for each subject
for subjNum1 = 2: 2%length (currentList)
    cd(path);
    ...
    for subjNum = subjNum1: subjNum1 %length (currentList)
        cd (path)
        iname = currentList (subjNum).name;
        disp(iname);
        cd (iname)
       ...
       save_avw(mask, 'mask_prediction_cv10_2.nii.gz','f',scales2);   
