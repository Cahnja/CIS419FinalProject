...
for subjNum = 1: length (currentList)
    ...
    fileList = dir ('*prediction*');
    t1 = fileList(1).name;
    disp(t1);
    combined = fileList(2).name;
    flair = 'mask_prediction_flair.nii.gz';
    ...
    t1_selected = sum(t1_mask(:) >= 1); %selected positive
    t1_true_selected = sum(label_mask(:) >= 1 & t1_mask(:) >= 1); % selected and positive
