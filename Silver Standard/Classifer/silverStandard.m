...
for subjNum = 1: length (currentList)
   ...
   mask = mask * -1;
   q1 = quantile(mask(inds), 0.7); disp(q1);
   inds1 = find(mask <= q1 | brainmask==0);
   %inds1 = find(brainmask==0);

   mask(inds1) = 0;
   mask(indsEdge) = 0;

   q1 = quantile(mask2(inds), 0.9);
   disp(q1);
   ...
   save_avw(mask, 'mask_prediction_silver.nii.gz','f',scales2);   
   save_avw(mask2, 'mask_prediction_flair_silver.nii.gz','f',scales2);   
   save_avw(mask3, 'mask_prediction_combined_silver.nii.gz','f',scales2);   
