...
for subjNum1 = 1: 1%length (currentList)
    ...
    for subjNum = 1: 1%length (currentList)
            ...
            for j = m+1: y - m
                for k = m+1: z - m
                    if brainmask(i,j,k) > 0
                        myMatrix = brain_image(i-m:i+m,j-m:j+m,k-m:k+m);
                        myFlair = flair_image(i-m:i+m,j-m:j+m,k-m:k+m);
                        prediction1 = trainedNet(myMatrix(:));
       ...
       %mask = mask * -1;
       mask1 = mask;
       %q1 = quantile(mask1(inds), 0.1);
       %disp(q1);
       inds1 = find(mask1 ~= 2 | brainmask==0);
       ...
       save_avw(mask, 'mask_prediction_silver_cv10_knn2.nii.gz','f',scales2);   
